...
from metric.utils.utils import split_into_paragraphs, dawon_split_into_sentences_batched_no_discards #,split_into_sentences_batched
...
class FENICE:
    def __init__(
        ...
        self.coref_model = (
            ...
            CoreferenceResolution(load_model=True)
        )
    ...
    def _score(self, sample_id: int, document: str, summary: str):
        ...
        score = self.args.weight_min * np.min([al["score"] for al in alignments]) + self.args.weight_mean * np.mean([al["score"] for al in alignments])
    ...
    def cache(self, documents, summaries):
        ...
        # 여기 주석해제하고 summary 확인하기ㄴ
        claims = self.cache_claims(summaries)
        self.cache_sentences(documents=documents, claims=claims)
        if self.use_coref:
            self.cache_coref(documents)
    ...
    def cache_sentences(self, documents, claims):
        # all_sentences = dawon_split_into_sentences_batched(
        #     documents, batch_size=512, return_offsets=True, claims = claims
        # )
        all_sentences = dawon_split_into_sentences_batched_no_discards(
            documents, batch_size=512, return_offsets=True, claims = claims, args=self.args
        )
        #print("here is cache_sentences") # 여기가 찐 문장 출력되는 곳이다.
        #print(all_sentences)
    ...
    def cache_claims(self, summaries):
        ...
        #print(claims_predictions)
        #### 여기 summary chunking하는 곳이다. here!!!!
    ...
    def load_alignment(
            ...
            if all([k in self.alignments_cache for k in alignments_ids]):
                ...
                alignment = None
                saved_score = None
                max_compare_score = -np.inf
                for i, (ent, contr, neut) in enumerate(scores):
                    ent, contr, neut = ent.item(), contr.item(), neut.item()
                    compare_score = ent - self.args.weight_compare_cont * contr          
                    current_saved_score = ent - self.args.weight_cont * contr  

                    if compare_score > max_compare_score:
                        max_compare_score = compare_score
                        alignment = (premises[i], [ent, contr, neut])
                        saved_score = current_saved_score
