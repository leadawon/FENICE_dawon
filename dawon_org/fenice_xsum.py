...
import time
fenice = FENICE(use_coref=True)

result_str = '../results/aggre_xsum/fenice_original_papersetting.json'

# Load the xsumfaith.json file
with open('../data/aggre_fact_xsum_sota.json', 'r', encoding="utf-8") as f:
...
for i, item in enumerate(factcc_data):
    print(f"Processing item {i + 1}/{len(factcc_data)}: {result_str}"*40)
